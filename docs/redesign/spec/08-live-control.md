...
| `dashboard/components/ChatSteerer.tsx` | Becomes the Chat desk. Add optional controlled `steer`/`onSteerChange` props, a per-line `outcome` and a `pendingLabel`. The logic and the direction throttle are unchanged; the only behaviour change is the `!frame`/`!snap` throttle (DEC-8-13, §8.5.9) | 8B |
| `dashboard/components/TrackManager.tsx` | Becomes the Dock › Audio deck. The `autoplay`/`autoplayDelay` props are already removed in 5C (§14.3 R5, D3), which also imports MorphSlider through `next/dynamic`; 8B does not re-add them. The MorphSlider keeps `onIndexChange={selectSliderTrack}` (DEC-8-04) and is slot-gated. The track list becomes a `role="radiogroup"`. Export a pure `TrackDeck`. Violet and the `:144` `backdrop-blur` are already gone in 1B (§5.10, §5.19 codemod 4), and the `:144` authored uppercase lands in M2 (§5.20.5) | 8B |
...
| Layer | Element | Spec |
|---|---|---|
| Bezel row 1 | `<TallyBar/>` (§7.4) | 28 px. Left: the `STBY`/`PVW` lamp, then the `REC` lamp with readout `REC {bytes} · HH:MM:SS` (§7.4; `bytes(n)`, §5.20.4). Right: the `ON AIR` lamp. It reads the broadcast store |
...
- **Body** (164 px, `id="lc-dock-body"`, `hidden` when collapsed):
  ...
  - **Audio:** track list (240), armed-track controls (`flex: 1`, min 280), artwork (148×148, shown only when the dock is at least 760 px wide).
...
| **8B: Instrument** | Grid, monitor anatomy, transport, telemetry, Session sheet, Rundown, Dock, the states in §8.6, keyboard, a11y, `live-control.css`, the LiveSkeleton swap in `app/admin/(live)/loading.tsx`, `LiveControlVisualFixture` and its page line, codemod 10 on every Live Control file (§5.19), the ScriptTemplatePicker restyle, DEC-8-13, the `FX_DECRYPT_LABELS` import, deleting `tests/live-extraction.spec.ts`, and the SKILL.md step 5 replacement | Full restyle | §8.10 B-series |
| **8C: Air** | `useWhipTruth` + `tests/whip-truth.spec.ts`; the go-live HoldButton + ConfirmDialog; the program keyline; `twitch.stop` registration, with the §6.7 stalled chyron verified end to end; the cue-failure chyron; `broadcast.setLeaveHandler`/`leave` wiring and the ScriptTemplatePicker link guards; the Stop confirm (DEC-8-14); end-to-end ON AIR title/favicon checks | ON AIR states only | §8.10 C-series |
...
| Broadcast store (§7.17) | `director`, `firstFrame`, `rec { active, startedAt, bytes }`, `air`, `airSince`, `stalledSince`, `whip`. DirectorPlayer reads `firstFrame` with `useBroadcast(s => s.firstFrame)` and passes it as a prop |
...
| Dock | `Tabs` (`keepMounted`), `IconButton`, `Checkbox`, `Slider` (native range), `Button`, `Skeleton`, `NotConfigured`/`AuthRequired` (`size="panel"`) |
...
- `premise` (8B): `connect()` sets `premise = prompt` right after `setState('opening')`. `connect` still sends `prompt` in `configure` (`:1013`). **Composer lifecycle (DEC-8-01):**
...
| WHIP installed (8C) | `TwitchBroadcast.tsx:152-153` | — | none; `setWhip(session)` hands over to `useWhipTruth` |
| Truth gate true, stall, recovery, WHIP stats (8C) | `useWhipTruth` | useWhipTruth | `on`, `stalled`, recovery `on`, `whip` (below) |
| Negotiation failed (8C) | `onNegotiationFailed` | useWhipTruth, then useTwitchBroadcast | useWhipTruth publishes `{ air: 'off', airSince: undefined, whip: undefined }`. useTwitchBroadcast then tears the WHIP session down without publishing `offair` (the `:130-133` teardown plus `setWhip(null)`) and pushes the cue-failure chyron: `chyron.push({ tone: 'error', title: "Couldn't start the Twitch broadcast", body: 'Twitch ingest never received media. Try again.' })` |
| Stop broadcast / session end (8C) | `TwitchBroadcast.tsx:129-135`, `:95-105` (only inside its `if (whipRef.current)` branch, so mount never publishes) | useTwitchBroadcast | `{ air: 'offair', whip: undefined, stalledSince: undefined }` and `setWhip(null)`; after 3000 ms, if `air` is still `offair`, `{ air: 'off', airSince: undefined }` (the timer is cleared on unmount) |
...
**`useWhipTruth(session: WhipSession | null, onNegotiationFailed: () => void): void`** (8C; it wraps `dashboard/lib/twitchWhip.ts` and never modifies it). §8.5.6 owns this algorithm; §6.7 keeps only the `air` state table. Poll every 1000 ms. On each poll:
...
3. **Stall and negotiation failure** (inside `nextAir`). While `air ∈ {on, stalled}`: `connectionState ∈ {failed, disconnected}`, or `bytes` flat for 4 consecutive polls, gives `stalled` (publish `{ air: 'stalled', stalledSince: Date.now() }` on entry); when the gate is true again, `on` (keep `airSince`). While `air === 'cue'`: `connectionState ∈ {failed, disconnected}`, or no truth gate within 10 000 ms of the session arriving, gives `off`: publish `{ air: 'off', airSince: undefined, whip: undefined }` and call `onNegotiationFailed()`. A `cue` session never becomes `stalled`.
...
6. When `session` becomes `null`: clear the interval, the listener and the op counter. useWhipTruth never publishes on mount, on unmount or on `null`.
7. Ignore late `getStats()` results with an op counter. Swallow errors. **Zero console output.**

Implement steps 2–3 as an exported pure reducer, `nextAir(prev: { air; lastBytes; flatPolls; airSince?; cueSince? }, sample: { connectionState; bytes; now }) → next`, where `cueSince` is the time the session arrived while `air === 'cue'`, so that `dashboard/tests/whip-truth.spec.ts` can check the gate without a browser (C3).

**BroadcastProvider** (§6.7, §6.12; lands in 4B) owns the ON AIR side effects: the title and favicon swap and restore, the announcements, the house lights, the stalled chyron ('Twitch ingest lost', action 'End broadcast') and its recovery, the off-air message, `beforeunload` and the mouse-button guard. §8 adds only the publishers above and the `twitch.stop` command, and verifies the result end to end (C1, C2).
...
| 2 | otherwise | `<Button variant="secondary" icon={Square}>Stop</Button>`, onClick `requestStop` (below) | `pending={state === 'closing'}` with **no** `pendingLabel` (the label stays 'Stop'; a BayerSpinner replaces the icon; clicks are guarded). DEC-8-02 fixes B12 | none |
...
| 8 | otherwise | `<HoldButton holdMs={600} variant="secondary" label="Go live on Twitch" confirm={…}>` (Radio icon; the confirm dialog is §8.6.5 step 2) | disabled when `!live \|\| !firstFrame \|\| !streamKeyReady \|\| starting`. Pending while `starting`, with label 'Negotiating…' (`:208`). DEC-8-03 adds `!firstFrame` | `live ? 'Push the live Director output to Twitch ingest' : 'Start the Director session first'` (`:205`) |

- **`requestStop` (DEC-8-14, 8C).** While `air ∈ {on, stalled}`, 'Stop' and the palette's `director.stop` open `<ConfirmDialog title="Stop the Director?" body="Stopping ends the session and the Twitch broadcast." confirmLabel="Stop and end broadcast" cancelLabel="Keep running" destructive onConfirm={disconnect}>`. Otherwise they call `disconnect()` at once, so the §1.6 flows are unchanged. Before 8C (no `air` yet), `requestStop` is `disconnect`.
- **Stop mount guard (DEC-8-14).** The 'Stop' control ignores activations for 600 ms after it mounts and ignores `keydown` with `event.repeat`, so a double-click or a held Enter on 'Start Director' never cancels the connect it just started.
...
- **Audio (TrackManager → deck).** It still `return null`s when `!useConvexEnabled()`; the Audio tab is then not rendered either.
  - **Header row:** h3 'Audio library' + kicker `COAST ORIGINALS · {n} TRACKS` (authored uppercase in M2, §5.20.5; its `backdrop-blur` went in 1B, §5.10; listed in §8.9.6) + 'Upload song' (`pending={uploading}`; the label stays 'Upload song').
  - **List:** `role="radiogroup" aria-label="Armed track"`. Each row is a flex of a `role="radio" aria-checked` button (32×32 cover `bg-screen rounded-screen` or a Music glyph, name, size) followed by `` <IconButton icon={Trash2} label={`Delete ${track.name}`}> `` **outside** the radio. Roving tabindex, with Space/Enter to arm. Clicking a radio arms it.
  ...
  - **Artwork:** `<section aria-label="Coast originals artwork carousel">` (148×148). It holds `<MorphSlider items activeIndex={armedSliderIndex} transition="melt" intensity={0.46} aberration={0.24} drift={0} radius={2} overlayColor="#05080F" onIndexChange={selectSliderTrack} />` **with no `autoplay`** (D3); `onIndexChange={selectSliderTrack}` is unchanged (`TrackManager.tsx:142`), so explicit artwork navigation arms that track (DEC-8-04). It is mounted only while `useEffectCanvasSlot('morph', 2, want)` returns true, where `want = audioPanelVisible && dockOpen && dockWidth >= 760 && sliderTracks.length > 0`. Otherwise the section shows a static `<img src={armedOrFirstCover} alt="">` (`rounded-screen`, `object-cover`).
...
| `morph` | **2** | Audio panel visible && dock open && dock width ≥ 760 && covered tracks > 0 | static cover `<img>` |
...
- **Constraints:**
  ...
  - No network (no `fetch` in fixture files), no session, no WebGL besides the carrier. Images and media are same-origin (`/brand/**`, `/fixtures/**`) or `data:`; `programStill` is one of these.
...
| Audio | 3 row skeletons + sr-only 'Loading…' | 'No songs yet — upload one to use it as the stream's audio reference.' | radiogroup + armed panel + artwork | 'Queue on next direction' disabled `!live`; `AuthRequired` when unauthenticated |
...
| Negotiation failed (no truth gate in 10 s, or the connection fails while `cue`) | unchanged; the cue-failure chyron "Couldn't start the Twitch broadcast" | 'Go live on Twitch' | `off` |
...
6. **Off air.** Triggered by 'Stop broadcast' or by the session ending (`:95-105`, with its opRef invalidation unchanged): `offair`, then after 3 s `off`. The title and favicon are restored (§6.7).
7. **Stopping the Director while on air (DEC-8-14).** 'Stop' or the palette's 'Stop' while `air ∈ {on, stalled}` first opens the 'Stop the Director?' dialog (§8.5.7). 'Stop and end broadcast' runs `disconnect()`, and the session end then takes the broadcast off air (step 6). 'Keep running' changes nothing.

> Note: the chyron action is 'End broadcast', not the bible's 'Stop broadcast'; the label and the reason for it are §6.7's.

#### 8.6.6 Air lock on Live Control (`html[data-lock="air"]`, §6.12 matrix)
...
|---|---|
| Dock | collapse control disabled, with the tooltip 'Locked while on air' (§6.12) |
...
| MorphSlider | explicit prev/next/dot/drag | tween only; rAF only during tween, drag or texture load (§12.4.1) | allowed only when it holds the slot (never while acquiring) | tween capped at 0.35 s (existing) |
...
| Track rows | `role="radio"` inside `role="radiogroup" aria-label="Armed track"` |
...
**Keyboard map** (no single-character global shortcuts, §7.12):
...
| ←/→ | MorphSlider stage (focused) | existing prev/next artwork (`MorphSlider.tsx` keyDown) |
...
| L13 | `disconnect` moves verbatim (§8.5.2). 'Stop' (directly, or through the DEC-8-14 dialog while on air), 'Cancel', Esc (`director.cancel`), the palette 'Stop' and "Leave and stop" all call that same function |
...
7. Behaviour decisions made by this spec, labelled `DEC-8-<nn>` (§1.7; a decision Devin makes in a PR uses `DEC-M<n>-<nn>`):
   - DEC-8-01: the composer lifecycle (§8.5.6).
   - DEC-8-02: 'Stop' is click-guarded while `closing`.
   - DEC-8-03: 'Go live on Twitch' also requires the first frame.
   - DEC-8-04: MorphSlider autoplay is off (D3: the `autoplay`/`autoplayDelay` props are removed in 5C, §14.3 R5). `onIndexChange={selectSliderTrack}` is kept, so explicit artwork navigation (prev/next, the 'Show {caption}' tabs, drag) arms that track, as at 845147c, and the carousel never shows or announces an unarmed track after user input.
   - DEC-8-05: the error can be dismissed.
   - DEC-8-06: the pushing line shows only when `air === 'on'`.
   - DEC-8-07: "Leave and stop" runs the full `disconnect()` before navigating.
   - DEC-8-08: `closed` after a first frame shows the standby slate ('Off air'); `closed` with no first frame in the attempt shows the FailedSlate ('Failed'). Neither shows 'Cancel'.
   - DEC-8-09: the log keeps 500 non-debug entries plus 100 debug entries, with debug hidden (the 50-line cap is removed).
   - DEC-8-10: track rows are radios, so clicking the armed row keeps it armed instead of disarming it.
   - DEC-8-11: beats are keyed by a client id.
   - DEC-8-12: the template picker shows a skeleton while loading.
   - DEC-8-13: `!frame`/`!snap` obey a 10 s global and 8 s per-user throttle (`lastFrameAtRef`, `lastFrameByUserRef`); a throttled frame command gets the `THROTTLED` badge. The direction throttle (`ChatSteerer.tsx:94-100`) is byte-identical.
   - DEC-8-14: while `air ∈ {on, stalled}`, 'Stop' and the palette's 'Stop' first open the 'Stop the Director?' dialog; otherwise Stop acts at once. 'Stop' ignores activations for 600 ms after it mounts and ignores `keydown` with `event.repeat`.
...
- **Copy:**
  ...
  - Dialogs: 'Go live on Twitch?', with the bodies 'Viewers on twitch.tv/{auth.login} will see program output.' and 'Viewers on the channel bound to this stream key will see program output.'; 'Leave Live Control?' with 'Leaving Live Control ends the Director session and the broadcast.'; 'Stop the Director?' with 'Stopping ends the session and the Twitch broadcast.'.
  - Chyron: the cue-failure chyron, title "Couldn't start the Twitch broadcast" and body 'Twitch ingest never received media. Try again.'. The stalled chyron ('Twitch ingest lost') is §6.7's.
...
- [ ] B11. Fixture screenshots of every §8.5.13 state in dark and light match the committed baselines (§15), and transport slot x-positions are identical between the idle and preview fixtures (no reflow).
- [ ] B12. `grep -c "autoplay" components/TrackManager.tsx` prints `0`, and `grep -c "onIndexChange={selectSliderTrack}" components/TrackManager.tsx` prints `1`. Every aria-label in the §8.9.4 TrackManager and MorphSlider rows is found by `grep -F`.
...
- [ ] B26. Configured and manual only (list under "Deferred / blocked" when there are no credentials): with Convex set, toggling the Audio tab 20 times prints no console output, and `window.__wzrd.slots` never shows two holders.
...
- [ ] B31. `grep -cE "lastFrameAtRef|lastFrameByUserRef" components/ChatSteerer.tsx` prints ≥ 4, and `git diff 845147c -- components/ChatSteerer.tsx | grep -E '^-.*(2000|8000|lastDirection)'` prints nothing (no direction-throttle line is removed or changed, DEC-8-13).
- [ ] B32. `npx playwright test --project=unit tests/unit/diagnostic.spec.ts` passes (the §15 `unit` project, no browser). It checks `formatDiagnostic`: a `network-path` progress event with `local: 'srflx/udp 203.0.113.7:54321'`, `remote: 'relay/udp 198.51.100.2:3478'`, `usesTurn: 'no'` and `rttMs: 42` gives `network-path · usesTurn no`; an `ice-servers` event with `source: 'ice-endpoint-failed: 203.0.113.7 timed out'` gives `ice-servers · source ice-endpoint-failed: [addr] timed out`; a value holding `2001:db8::1` is masked to `[addr]`; a `warning` event with message `x` gives `warning: x`.
...
- [ ] C3. `npx playwright test tests/whip-truth.spec.ts` passes. It exercises `nextAir` with: `cue` + connected + growing bytes → `on`; `on` + 4 flat polls → `stalled`; `on` + `connectionState: 'failed'` → `stalled`; then growth again → `on` with `airSince` unchanged; `cue` + `failed` → `off`; `cue` with 10 s since `cueSince` and no gate → `off`; `cue` with 4 flat polls under 10 s → `cue`.
...
- [ ] C7. Stop while on air (DEC-8-14). In the fixture's on-air state, clicking 'Stop' opens a dialog titled 'Stop the Director?' with the body 'Stopping ends the session and the Twitch broadcast.' and the buttons 'Keep running' and 'Stop and end broadcast'. In the preview state, clicking 'Stop' opens no dialog.
