...
export const prepare = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [scenes, allShots, characters] = await Promise.all([
      ...
      ctx.db.query('characters').withIndex('by_board', (q) => q.eq('boardId', args.boardId)).collect(),
    ])
    ...
    for (const shot of sorted) {
      ...
      const context = scene ? [scene.title, scene.location, scene.timeOfDay, scene.weather, scene.atmosphere].filter(Boolean).join(' · ') : ''
