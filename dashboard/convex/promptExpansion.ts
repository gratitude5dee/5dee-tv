...
export const expand = action({
  ...
  handler: async (ctx, args): Promise<{ prompt: string; notes?: string; requestId: string; templateVersion: string; coordinatorModel: string; workerModel: string }> => {
    ...
    const coordinator = process.env.GMI_COORDINATOR_MODEL || DEFAULT_COORDINATOR; const worker = process.env.GMI_WORKER_MODEL || DEFAULT_WORKER; const base = `${args.source}\n\nContext:\n${args.context || '(none)'}`
