...
export const list = query({ args: {}, handler: async (ctx) => { if (!(await ctx.auth.getUserIdentity())) return []; return await ctx.db.query('styles').withIndex('by_updatedAt').order('desc').take(100) } })
