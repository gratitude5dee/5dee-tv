...
function CharacterLibraryInner() {
  ...
  const [draft, setDraft] = useState({ name: '', handle: '', description: '', imageUrl: '' })
  const draftRef = useRef(draft)
  const draftOwnerRef = useRef<Id<'characters'> | null>(null)

  useEffect(() => {
    if (!current || draftOwnerRef.current === current._id) return
    const next = { name: current.name, handle: current.handle ?? '', description: current.description ?? '', imageUrl: current.imageUrl ?? '' }
  ...
  const updateDraft = (changes: Partial<typeof draft>) => {
    ...
    draftRef.current = next
    setDraft(next)
    if (current) void save({ characterId: current._id, ...next })
  }
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(220px,0.7fr)_minmax(0,1.3fr)]">
      ...
      <div className="fal-card"><div className="fal-card-content space-y-3">{current ? <><div className="grid gap-3 sm:grid-cols-2"><label className="text-xs text-fal-gray-500">Name<input className={field} value={draft.name} onChange={(e) => updateDraft({ name: e.target.value })} /></label><label className="text-xs text-fal-gray-500">Handle (without @)<input className={field} value={draft.handle} onChange={(e) => updateDraft({ handle: e.target.value.replace(/^@/, '').toLowerCase().replace(/[^a-z0-9-_]/g, '') })} /></label></div><label className="text-xs text-fal-gray-500">Identity and continuity description<textarea className={field} rows={6} value={draft.description} onChange={(e) => updateDraft({ description: e.target.value })} placeholder="Appearance, wardrobe, age, personality, and invariants…" /></label><AssetUrlInput value={draft.imageUrl} onChange={(url) => updateDraft({ imageUrl: url })} placeholder="Character sheet or portrait URL" /><p className="flex items-center gap-1 text-xs text-fal-gray-500">{saving ? <><Loader2 className="w-3 h-3 animate-spin" /> Saving…</> : <><Save className="w-3 h-3" /> Saved automatically</>}</p></> : <p className="text-sm text-fal-gray-500">Choose a character or create one to edit its identity references.</p>}{error && <p className="text-xs text-red-600">{error}</p>}</div></div>
    </div>
  </div>
}
