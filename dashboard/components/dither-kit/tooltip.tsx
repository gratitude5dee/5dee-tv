...
const VARIANT: Record<TooltipVariant, string> = {
  ...
  "frosted-glass": "bg-white/70 backdrop-blur-sm dark:bg-gray-950/70",
}
    ...
    <AnimatePresence>
      {show && items.length > 0 && (
        <motion.div
          ...
          className={cn(
            "pointer-events-none absolute z-10 rounded-md border border-gray-200 px-2 py-1 shadow-sm dark:border-gray-800",
            VARIANT[variant]
          )}
        >
          {heading && (
            <div className="mb-0.5 font-mono text-[10px] text-gray-500 dark:text-gray-400">
              {heading}
            </div>
          )}
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-1.5 font-mono text-[11px] text-gray-950 tabular-nums dark:text-gray-50"
              ...
              >
                ...
                <span className="ml-auto pl-2 text-gray-950 dark:text-gray-50">
                  {valueFormatter
