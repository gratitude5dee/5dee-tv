    ...
    >
      {Object.entries(config).map(([name, entry]) => {
        ...
        return (
          <li
            key={name}
            className="flex items-center gap-1.5 font-mono text-[11px] text-gray-500 dark:text-gray-400"
          >
            <span
            ...
            {value !== undefined ? (
              <span className="text-gray-950 dark:text-gray-50">{valueFormatter(value)}</span>
            ) : null}
          </li>
        )
      })}
