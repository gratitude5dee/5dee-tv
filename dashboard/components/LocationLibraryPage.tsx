...
function LocationLibraryInner() {
  ...
  const update = async (args: Parameters<typeof patch>[0]) => { setSaving(true); try { await patch(args) } finally { setSaving(false) } }
  const add = async () => { const id = await create({ name: 'New location' }); setSelected(id) }

  return <div className="space-y-4"><div className="fal-card"><div className="fal-card-header flex items-center justify-between"><div><h1 className="fal-card-title flex items-center gap-2"><MapPin className="w-4 h-4" /> Location library</h1><p className="text-xs text-fal-gray-500 mt-1">Reusable environments selectable on storyboard scenes.</p></div><button type="button" onClick={() => void add()} className="fal-button-secondary flex items-center gap-1 text-xs"><Plus className="w-3.5 h-3.5" /> New location</button></div></div><div className="grid gap-4 lg:grid-cols-[minmax(220px,0.7fr)_minmax(0,1.3fr)]"><div className="fal-card"><div className="fal-card-content p-2 space-y-1">{locations?.filter((l) => !l.archivedAt).map((location) => <button type="button" key={location._id} onClick={() => setSelected(location._id)} className={`w-full rounded-md border px-3 py-2 text-left text-sm ${selected === location._id ? 'border-fal-primary-500 bg-fal-primary-50 dark:bg-fal-primary-900/20' : 'border-fal-gray-200 dark:border-fal-gray-700'}`}>{location.name}</button>)}</div></div><div className="fal-card"><div className="fal-card-content space-y-3">{current ? <><label className="text-xs text-fal-gray-500">Name<input className={field} value={current.name} onChange={(e) => void update({ locationId: current._id, name: e.target.value })} /></label><label className="text-xs text-fal-gray-500">Environment description<textarea className={field} rows={6} value={current.description ?? ''} onChange={(e) => void update({ locationId: current._id, description: e.target.value })} placeholder="Architecture, materials, light, weather, and style invariants…" /></label><AssetUrlInput value={current.imageUrl ?? ''} onChange={(url) => void update({ locationId: current._id, imageUrl: url })} placeholder="Location reference URL" /><p className="flex items-center gap-1 text-xs text-fal-gray-500">{saving ? <><Loader2 className="w-3 h-3 animate-spin" /> Saving…</> : <><Save className="w-3 h-3" /> Saved automatically</>}</p></> : <p className="text-sm text-fal-gray-500">Choose a location or create one to edit its environment reference.</p>}</div></div></div></div>
}
